local path_sep = require("folk").path_sep
local O = require("folk").options
local M = {}
...
function M.compile(flavour)
	local theme = require("folk.lib.mapper").apply(flavour)
	local lines = {
		string.format(
			[=[
...
let g:colors_name = "folk-%s"]=],
	...
	if vim.g.folk_debug then -- Debugging purpose
	...
	if not f then
		local err_path = (path_sep == "/" and "/tmp" or os.getenv "TMP") .. "/folk_error.lua"
		print(string.format(
			[[Folk (error): Most likely some mistake made in your folk config
